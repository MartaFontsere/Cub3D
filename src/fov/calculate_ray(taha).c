...
/*   Updated: 2025/03/19 22:19:22 by mfontser         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ray_gose_through_door(t_ray_hit new, t_ray_hit old, t_ray *ray, t_game *gdata)
{
    double old_x;
    double old_y;
    double new_x;
    ...
    if (ray->y_sign > 0)
        old_y = compute_collision_coordinate(old.check_ray_y_in_map, ray->y_sign, gdata->minimap.px_in_cell_height) + (gdata->minimap.px_in_cell_height / 2);
    else
        old_y = compute_collision_coordinate(old.check_ray_y_in_map, ray->y_sign, gdata->minimap.px_in_cell_height) - (gdata->minimap.px_in_cell_height / 2);
    if (ray->x_sign > 0)
        old_x = compute_collision_coordinate(old.check_ray_x_in_map, ray->x_sign, gdata->minimap.px_in_cell_width) + (gdata->minimap.px_in_cell_width / 2);
    else
        old_x = compute_collision_coordinate(old.check_ray_x_in_map, ray->x_sign, gdata->minimap.px_in_cell_width) - (gdata->minimap.px_in_cell_width / 2);

    new_y = compute_collision_coordinate(new.check_ray_y_in_map, ray->y_sign, gdata->minimap.px_in_cell_height);
    new_x = compute_collision_coordinate(old.check_ray_x_in_map, ray->x_sign, gdata->minimap.px_in_cell_width);

    if (ray->line_crossing == 0)
        old_distance = ((old_x / gdata->minimap.px_in_cell_width) - gdata->player.cell_player_x) / ray->dir_x;
    else
        old_distance = ((old_y / gdata->minimap.px_in_cell_height) - gdata->player.cell_player_y) / ray->dir_y;
    
    if (ray->line_crossing == 0)
        new_distance = ((new_x / gdata->minimap.px_in_cell_width) - gdata->player.cell_player_x) / ray->dir_x;
    else
        new_distance = ((new_y / gdata->minimap.px_in_cell_height) - gdata->player.cell_player_y) / ray->dir_y;

    old_distance = old_distance * cos(ray->current_angle - gdata->vision.vision_angle);
    new_distance = new_distance * cos(ray->current_angle - gdata->vision.vision_angle);
    if (new_distance < old_distance)
        return (1);
    else
        return (0);
    old_y -= gdata->player.y;
    new_y -= gdata->player.y;
    old_x -= gdata->player.x;
    new_x -= gdata->player.x;
    old_distance = sqrt(pow(old_y, 2.0) + pow(old_x, 2.0));
    new_distance = sqrt(pow(new_y, 2.0) + pow(new_x, 2.0));
    if (new_distance > old_distance)
        return (1);
    return (0);
}
...
int traverse_ray_until_hit(t_ray *ray, t_game *gdata, int *check_ray_x_in_map, int *check_ray_y_in_map)
{
    ...
    while (wall_hit == 0) 
    {
        ...
        {
            old.line_crossing = ray->line_crossing;
            old.first_dist_x = ray->first_dist_x;
            old.first_dist_y = ray->first_dist_y;
            old.check_ray_x_in_map = *check_ray_x_in_map;
            old.check_ray_y_in_map = *check_ray_y_in_map;
            ...
            new.line_crossing = ray->line_crossing;
            new.first_dist_x = ray->first_dist_x;
            new.first_dist_y = ray->first_dist_y;
            new.check_ray_x_in_map = *check_ray_x_in_map;
            new.check_ray_y_in_map = *check_ray_y_in_map;

            if (ray_gose_through_door(new, old, ray, gdata))
            {
                ray->first_dist_y = old.first_dist_y;
                *check_ray_y_in_map = old.check_ray_y_in_map;
                ray->first_dist_x = old.first_dist_x;
                *check_ray_x_in_map = old.check_ray_x_in_map;
                ray->line_crossing = old.line_crossing;
            }
            // if (line_crossing_tmp == 1 && ray->line_crossing == 1)
            // {
            //     printf("this is the strange case: 3: %f %f\n", ray->first_dist_y, first_dist_y_tmp);
            //     ray->first_dist_y = first_dist_y_tmp;
            ...
            // }
            // else if (line_crossing_tmp == 1 && ray->y_sign < 0)
            // {
            //     num = ray->first_dist_y - floor(ray->first_dist_y);
            //     printf("this is the strange case: 2\n");
            //     if (num < 0.5)
            //     {
            //         ray->first_dist_x = first_dist_x_tmp;
            //         *check_ray_x_in_map = check_ray_x_in_map_tmp;
            ...
            // {
            //     num = 0.2;
            //     printf("this is the strange case: 1: %f %f\n", ray->first_dist_x, first_dist_x_tmp);
            //     printf("this is the strange case: 1: %f\n", num);
            //     printf("this is the strange case: 1: %f other\n", ray->other_dist_x);
            //     printf("this is the strange case: 1: %f other\n", ray->other_dist_y);
            //     if (num > 0.5)
            //     {
            //         ray->first_dist_x = first_dist_x_tmp;
...
int calculate_ray(t_game *gdata, t_ray *ray, double x, double y)
{
   ...
   ray->check_ray_y_in_map = (int)(y / gdata->minimap.px_in_cell_height);// Representa la celda en la cuadrícula donde está el rayo  (índices de la matriz del mapa). Empieza en la casilla del player
    ...
    if (traverse_ray_untdil_hit(ray, gdata, &ray->check_ray_x_in_map, &ray->check_ray_y_in_map) == 0)
    {
        //FREE DATA
        return (0);
    }
