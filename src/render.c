...
void render_game (void *param) // todo lo que se tenga que checkear a cada vuelta de renderizado
{
	...
	gdata->player.ray.last_vision_angle = gdata->player.ray.vision_angle; // lo actualizo siempre, porque si solo lo actualizo al rotar, cuando solo se mueve no se actualiza el angulo anterior, que debe ser el mismoo que el actual
