...
/*   Updated: 2025/04/04 00:13:51 by mfontser         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_player(t_game *gdata, t_player player, int x, int y)
{
	...
	while (draw_y <= y_limit)
	{
		...
		{
			if ((draw_x - x) * (draw_x - x) + (draw_y - y) * (draw_y
					- y) <= player.radius * player.radius)
			{
				mlx_put_pixel(gdata->mlx.mini_image, (int)draw_x, (int)draw_y,
					RED1);
			}
			draw_x++;
		}
