...
void	draw_cell_pixel(t_game *gdata, char cell, int pixel_x, int pixel_y)
{
	...
	else if (cell == '0' || cell == 'N' || cell == 'S' || cell == 'W'
		|| cell == 'E')
...
{
	if (matrix_cell_x >= 0 && matrix_cell_x < gdata->map.c_width
		&& matrix_cell_y >= 0 && matrix_cell_y < gdata->map.c_height)
		return (1);
	return (0);
}
...
int	is_out_of_bounds(t_game *gdata, int pixel_x, int pixel_y)
{
	...
	map_px_width = gdata->minimap.px_in_cell_width * gdata->map.c_width;
	map_px_height = gdata->minimap.px_in_cell_height * gdata->map.c_height;
	if (gdata->print_minimap.start_x + pixel_x < 0
		|| gdata->print_minimap.start_y + pixel_y < 0)
		return (1);
	if (gdata->print_minimap.start_x + pixel_x >= map_px_width
		|| gdata->print_minimap.start_y + pixel_y >= map_px_height)
		return (1);
	return (0);
}
