...
/*   Updated: 2025/04/04 03:34:29 by mfontser         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		double origin_y)
{
	...
	map_x = (int)((ray->draw.current_x - origin_x + gdata->player.x)
			/ gdata->minimap.px_in_cell_width);
	map_y = (int)((ray->draw.current_y - origin_y + gdata->player.y)
...
void	draw_ray_pixel(t_game *gdata, t_ray *ray, int current_step)
{
	...
	px_x = (int)(ray->draw.current_x + 0.5);
	px_y = (int)(ray->draw.current_y + 0.5);
	if (px_x >= 0 && px_x < gdata->minimap.px_width && px_y >= 0
		&& px_y < gdata->minimap.px_height)
	{
...
void	draw_ray(t_game *gdata, t_ray *ray, double origin_x, double origin_y)
{
	...
	ray->draw.steps = fmax(fabs(ray->draw.x_distance),
			fabs(ray->draw.y_distance));
