...
/*   Updated: 2025/04/01 11:57:50 by mfontser         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_y_pos(int i, int j, t_map *map, t_game *gdata)
{
	...
	{
		if (map->matrix[i][j + 1] != '0' && map->matrix[i][j + 1] != gdata->player.orientation)
	...
	else if (map->matrix[i][j - 1] == gdata->player.orientation)
	{
		if (map->matrix[i][j + 1] != '0')
...
int	check_x_pos(int i, int j, t_map *map, t_game *gdata)
{
	...
	if (map->matrix[i - 1][j] != '0' && map->matrix[i - 1][j] != gdata->player.orientation)
...
int	is_door(int i, int j, t_map *map, t_game *gdata)
{
	...
	if (map->matrix[i][j] == 'D')
	{
		...
		if (map->matrix[i][j - 1] == '0' || map->matrix[i][j - 1] == gdata->player.orientation) // CARACTER  PLAYER
		{
			if (!check_y_pos(i, j, map, gdata))
				return (0);
		}
