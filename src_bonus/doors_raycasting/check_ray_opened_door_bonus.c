...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   check_ray_opened_door_bonus.c                      :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: mfontser <mfontser@student.42.barcel>      +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/26 21:42:21 by mfontser          #+#    #+#             */
/*   Updated: 2025/04/03 16:54:08 by mfontser         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	take_hit_door_position(t_ray *ray, t_game *gdata)
{
	...
	if (ray->line_crossing == 0)
	{
		...
		wall_x = fmod(ray->px_collision_y, gdata->minimap.px_in_cell_height)
	...
	else
	{
		...
		wall_x = fmod(ray->px_collision_x, gdata->minimap.px_in_cell_width)
...
int	should_block_ray(t_ray *ray, t_game *gdata, int map_x, int map_y)
{
	...
	shifted_tex_x = convert_hit_position_to_text_coord(gdata, wall_x,
			open_ratio);
